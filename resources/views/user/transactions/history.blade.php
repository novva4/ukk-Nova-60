                ...
                <tbody>
                    @forelse($transactions as $index => $trx)
                        <tr>
                            ...
                            <td>
                                <div style="font-weight:600; font-size:14px; color:var(--text-main);">{{ $trx->book->title }}
                                </div>
                                <div style="font-size:12px; color:var(--text-muted);">{{ $trx->book->author }}</div>
                            </td>
                            <td style="font-size:13px; color:var(--text-muted);">
                                {{ \Carbon\Carbon::parse($trx->borrow_date)->format('d M Y') }}</td>
                            <td style="font-size:13px; color:var(--text-muted);">
                                {{ \Carbon\Carbon::parse($trx->return_date)->format('d M Y') }}
                                @if ($trx->status === 'borrowed' && now()->gt($trx->return_date))
                                    <div style="font-size:11px; color:var(--danger); font-weight:600;">⚠ Melewati batas!
                                    </div>
                                @endif
                            </td>
                            <td>
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="empty-state">
                                    ...
                                    <p style="color:var(--text-muted);margin-top:12px;">Anda belum memiliki riwayat
                                        peminjaman.</p>
                                    <a href="{{ route('user.catalog') }}" class="btn-primary-custom"
